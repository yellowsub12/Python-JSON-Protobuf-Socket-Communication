...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s: # Creates socket object that supports context manager type, so you can use it in a with statement without needed s.close():
    ...
    with conn:
        ...
        while True: #infinite loop used to loop over blocking calls
            ...
            typeName = BenchmarkType(server_request['Benchmark_type'], server_request['Data_type'])
            ProcessedData = ProcessData(typeName, server_request['BatchUnit'], server_request['BatchSize'], server_request['BatchID'], server_request['WorkloadMetric'], server_request['DataAnalytics'])
            FinalBatchID = server_request['BatchID'] + server_request['BatchSize'] - 1
            ResponseForData = {"RFW_ID": server_request['RFW_ID'], "FinalBatchID": FinalBatchID, "ProcessedData": ProcessedData} 
